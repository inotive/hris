...
class EmployeeTemplateDataSheet implements FromCollection, WithHeadings, WithEvents, WithStrictNullComparison, WithTitle, WithStyles, ShouldAutoSize
{
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // --- VALIDATION USING 'Options' Sheet ---
                // We assume Options Sheet has:
                // A: Companies, B: Departments, C: Positions, D: Levels, E: Shifts
                // F: Genders, G: Religions, H: Marital Status, I: Status, J: Yes/No

                // Status (E) -> Col I
                $this->addValidation($sheet, 'E4:E' . $rowCount, 'Options!$I$1:$I$2');

                // Gender (H) -> Col F
                $this->addValidation($sheet, 'H4:H' . $rowCount, 'Options!$F$1:$F$10');

                // Religion (K) -> Col G
                $this->addValidation($sheet, 'K4:K' . $rowCount, 'Options!$G$1:$G$10');

                // Marital Status (L) -> Col H
                $this->addValidation($sheet, 'L4:L' . $rowCount, 'Options!$H$1:$H$10');

                // Company (U) -> Col A (Large List)
                $this->addValidation($sheet, 'U4:U' . $rowCount, 'Options!$A$1:$A$5000'); 

                // Department (V) -> Col B
                $this->addValidation($sheet, 'V4:V' . $rowCount, 'Options!$B$1:$B$1000');

                // Position (W) -> Col C
                $this->addValidation($sheet, 'W4:W' . $rowCount, 'Options!$C$1:$C$1000');

                // Level (X) -> Col D
                $this->addValidation($sheet, 'X4:X' . $rowCount, 'Options!$D$1:$D$1000');

                // Shift (Y) -> Col E
                $this->addValidation($sheet, 'Y4:Y' . $rowCount, 'Options!$E$1:$E$1000');

                // Document Unlimited (AD) -> Col J (Yes/No)
                $this->addValidation($sheet, 'AD4:AD' . $rowCount, 'Options!$J$1:$J$2');
                
                // Config (AL-AR) -> Col J (Yes/No)
                $this->addValidation($sheet, 'AL4:AL' . $rowCount, 'Options!$J$1:$J$2'); // Attendance Loc
                $this->addValidation($sheet, 'AM4:AM' . $rowCount, 'Options!$J$1:$J$2'); // Leave Req
                $this->addValidation($sheet, 'AN4:AN' . $rowCount, 'Options!$J$1:$J$2'); // Overtime Req
                $this->addValidation($sheet, 'AO4:AO' . $rowCount, 'Options!$J$1:$J$2'); // Reimb Req
                $this->addValidation($sheet, 'AP4:AP' . $rowCount, 'Options!$J$1:$J$2'); // Attendance
                $this->addValidation($sheet, 'AQ4:AQ' . $rowCount, 'Options!$J$1:$J$2'); // Payslip
                $this->addValidation($sheet, 'AR4:AR' . $rowCount, 'Options!$J$1:$J$2'); // EWA
            },
        ];
    }
    ...
    private function addValidation($sheet, $cellRange, $formula)
    {
        ...
        $validation->setShowErrorMessage(false); // Allowing manual input
