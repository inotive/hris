<x-table.index>
    ...
    <x-slot name="toolbar">
        <x-table.add-button :label="__('Add Subscription')" :href="route('company-subscriptions-detail.create', $company->id)" />
    </x-slot>
                    ...
                    <x-slot name="body">
                        @foreach ($list as $key => $value)
                            <tr>
                                ...
                                <td>{{ \App\Helpers\DateFormatHelper::format($value->start_date_at) }}</td>
                                <td>{{ \App\Helpers\DateFormatHelper::format($value->end_date_at) }}</td>
                                <td>{{ $value->payment_status == 1 ? __('Yes') : __('No') }}</td>

                                <td class="text-end">
                                    <x-table.actions>
                                        <x-table.edit-button :href="route('company-subscriptions-detail.edit', [$company, $value->id])" />
                                        <x-table.delete-button :href="route('company-subscriptions-detail.destroy', [$company, $value->id])"  />
                                    </x-table.actions>
                                </td>
