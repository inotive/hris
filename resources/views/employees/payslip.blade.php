<x-form.index :action="$form_action" :cancel="$cancel">
  ...
  <x-slot name="back">
    <x-back
      href="{{ route(explode('.', Route::currentRouteName())[0] . '.index') }}" />
  </x-slot>
  ...
  <x-slot name="body">
    <div class="row">
      <x-form.select
        :list="App\Models\BankList::pluck('name', 'name')"
      ...
      <x-form.select
        :list="$ptkp_list"
        label="Jenis PTKP (TER)"
        name="type_ter"
        :value="$employee->type_ter ?? ''" />

      <x-form.select
        :list="['hourly' => 'Hourly', 'daily' => 'Daily']"
        label="Overtime Type"
        name="overtime_type"
        :value="$employee->overtime_type ?? ''" />
      <x-form.currency
        type="text"
        label="Overtime Value"
        name="overtime_value"
        :value="$employee->overtime_value ?? ''" />

      <x-form.select
        :list="['monthly' => 'Monthly', 'yearly' => 'Yearly']"
        label="Reimbursement Type"
        name="reimbursement_type"
        :value="$employee->reimbursement_type ?? ''" />

      <x-form.currency
        :label="__('Reimbursement Value')"
        name="reimbursement_limit"
        :value="old('reimbursement_limit', $employee->reimbursement_limit ?? '')" />
    </div>

    <input
      type="hidden"
      id="deduction_details_data"
      value="{{ json_encode($deduction_details ?? []) }}" />
    <input
      type="hidden"
      id="earning_details_data"
      value="{{ json_encode($earning_details ?? []) }}" />
