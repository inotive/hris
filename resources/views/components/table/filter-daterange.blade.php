...
<div class="{{ $class }}" id="{{ $name }}_div">
    ...
    <div class="input-group input-group-solid mb-3">
        ...
        name="{{ $name }}" value="{{ isset(request()->daterange) ? \Carbon\Carbon::parse(explode(' - ', request()->daterange)[0])->format('d/m/Y') . ' - ' . \Carbon\Carbon::parse(explode(' - ', request()->daterange)[1])->format('d/m/Y') : date('01/m/Y') . ' - ' . date('t/m/Y') }}">
