<x-card>
  ...
  <x-slot name="header">
    <b>{{ __("Data Reimbursement Requests") }}</b>
  </x-slot>

  <x-slot name="toolbar">
    <x-table.filter-dropdown :company="true" :daterange="true" />
    <x-table.add-button
      :label="__('Add Reimbursement Request')"
      :href="route('reimbursement-requests.create')" />
  </x-slot>
        ...
        <x-slot name="body">
          ...
          <tr>
            ...
            <td class="text-end">
              <x-table.actions>
                <x-table.edit-button
                  :href="route('reimbursement-requests.edit', [$value->id])" />
                <x-table.delete-button
                  :href="route('reimbursement-requests.destroy', [$value->id])" />
              </x-table.actions>
            </td>
          </tr>
          @endforeach
